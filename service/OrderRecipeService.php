...
class OrderRecipeService {
    ...
    public function updateItem($itemId, $quantity){
         ...
         $this->conn->update("UPDATE `order_item` SET `quantity`=? WHERE `id`=? LIMIT 1", 
                 array($quantity, $itemId));
    }
    ...
    public function getRecipeItemsBy($itemId, $orderId){
        return $this->conn->select("SELECT i.`id`, oi.`id` as id2,  c.`name`, c.`code`, i.`price`, oi.`price_sale`, i.`quantity_kg`, m.`unit`, m.`id` as id_unit, oi.`quantity`
